...
async function main() {
  try {
    ...
    const artist = await Artist.findOne().exec();
    ...
    console.log(`✅ Found artist: ${artist.name} (ID: ${artist._id})`);
    ...
    // Verify
    const count = await OpenVerseCampaign.countDocuments().exec();
    console.log(`📊 Total campaigns in DB: ${count}`);
    
    const allCampaigns = await OpenVerseCampaign.find().exec();
